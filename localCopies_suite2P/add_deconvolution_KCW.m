function add_deconvolution_KCW(ops, db)
...
for i = 1:length(ops.planesToProcess)
    ...
    [sp, deconvCa , coefs,~, sd, ops, kernels] = wrapperDECONV(ops, Ff, Fneu);
    % [sp, ca, coefs, B, sd, ops]
    ...
    for j = 1:length(dat.Fcell)
       ...
       dat.ca{j} = deconvCa(nCum + [1:size(dat.Fcell{j},2)], :)';
       dat.kernels{j} = kernels;
       if ops.deconvNeuropil
