...
% npilSubTraces = npilSubTraces-nanmean(npilSubTraces);
...
[bf,af] = butter(2,7/fr/2,'low'); % 10 is good for low pass
...
for ff = 1:length(index)%1:size(raster,3)%
    ...
    if plotFig==1
        ...
        for ii=1:size(psth,1)
            plot(time,filtfilt(bf,af,psth(ii,:))','-','LineWidth',2,'Color',cs(ii,:));
